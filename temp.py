...
#### Put your code here
def randomized_smoothing_predict_fn(model, x):

    #ALL THE VALUES HERE WILL BE IN THE RANGE 0-1. SINCE I THINK THE VALUES OF ALL VALUES OF PIXELS HAVE BEEN NORMALIZED TO 0-1.
    # sigma=10/255
    gaussian_noise=np.random.normal(0.5,0.5)
    x_noisy=x+gaussian_noise
    x_noisy_clipped = tf.clip_by_value(x_noisy, 0,1)
...
if __name__ == "__main__":
    ...
    predict_fn = lambda x: randomized_smoothing_predict_fn(model, x)
